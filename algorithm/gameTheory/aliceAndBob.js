...
function sillyGame(n) {
    function initialize(upperN) {
        ...
        let count = 0;
        primeCountArr[0] = count;
        primeCountArr[1] = count;
        for (let i = 2; i <= upperN; i++) {
            if (isPrime(i) === true) {
                count++;
            }

            primeCountArr[i] = count;
        }
    }

    if (primeCountArr.length === 0)
        initialize(Math.pow(10, 5));

    let primeCount = primeCountArr[n];
